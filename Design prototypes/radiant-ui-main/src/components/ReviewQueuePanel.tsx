...
const mockItems: ReviewItem[] = [
  { id: "1", type: "conflict", title: "Pricing model contradicts docs", description: "Two sources disagree on the freemium tier structure.", product: "Slack" },
  { id: "2", type: "conflict", title: "Target market mismatch", description: "Landing page says SMB but case studies target enterprise.", product: "Stripe" },
  { id: "3", type: "sensitive", title: "Revenue figures detected", description: "ARR data found in uploaded pitch deck requires approval.", product: "Vercel" },
  { id: "4", type: "sensitive", title: "Employee count", description: "Headcount data flagged as sensitive.", product: "Linear" },
  { id: "5", type: "stale", title: "Pricing page outdated", description: "Source last verified 90+ days ago.", product: "Notion" },
];
...
const ReviewQueuePanel = ({ open, onClose, orgLevel = false }: Props) => {
                    ...
                    <div className="space-y-2">
                      {group.map((item) => (
                        <motion.div
                          ...
                          exit={{ opacity: 0, x: 50 }}
                          className={`rounded-xl bg-secondary/40 ring-1 ring-border/50 p-4 border-l-[3px] ${minimal ? "border-l-border" : `border-l-${cfg.dot.replace("bg-", "")}`}`}
                          style={{ borderLeftColor: minimal ? undefined : `var(--tw-${type})` }}
                        >
