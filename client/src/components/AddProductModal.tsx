...
const AddProductModal = ({ open, onClose, orgId }: Props) => {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const data = await res.json() as { product: { id: number; name: string } } | { id: number; name: string };
      const product = "product" in data ? data.product : data;
      if (selected) {
        await apiRequest("POST", `/api/products/${product.id}/type`, { productType: selected });
      }
      return product;
    },
    onSuccess: (product) => {
      queryClient.invalidateQueries({ queryKey: ["/api/products"] });
      queryClient.invalidateQueries({ queryKey: ["/api/products", orgId] });
      setTimeout(() => {
        handleClose();
                  ...
                  return (
                    <button
                      ...
                      className={`w-full rounded-xl p-4 text-left transition-all duration-200 ring-1 flex items-start gap-4 group ${
                        isSelected
                          ? minimal ? "bg-secondary ring-foreground/40" : `${c.bg} ${c.ring}`
