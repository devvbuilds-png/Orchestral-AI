...
import type { PKB, Persona, ICP } from "@shared/schema";
...
const PERSONA_TYPE_COLORS: Record<string, ColorKey> = {
  buyer_persona:   "cyan",
  user_persona:    "purple",
  influencer:      "pink",
...
const ICP_COLORS: ColorKey[] = ["blue", "purple", "pink", "amber"];

interface Props {
  pkb: PKB | null;
}

const PersonasTab = ({ pkb }: Props) => {
  const { minimal } = useMinimalMode();
  const cMap = minimal ? minimalColorMap : colorMap;

  const personas: Persona[] = pkb?.personas ?? [];
  const icps: ICP[] = pkb?.icps ?? [];
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full">
      ...
      {personas.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-8">
          {personas.map((persona, i) => {
            const colorKey = PERSONA_TYPE_COLORS[persona.type] ?? "cyan";
            const c = cMap[colorKey];
            const colorDark = colorMap[colorKey];
            const confidencePct = Math.round((persona.confidence ?? 0) * 100);
            const typeLabel = PERSONA_TYPE_LABELS[persona.type] ?? persona.type;
            const isInferred = persona.lifecycle_status === "inferred";
            return (
              <motion.div
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-secondary text-foreground">{typeLabel}</span>
                        {isInferred && (
                  ...
                  <div className="text-right">
                    <span className="text-sm font-extrabold text-foreground">{confidencePct}%</span>
                    <div className="w-20 h-1.5 rounded-full bg-muted mt-1 overflow-hidden">
                      <div className={`h-full rounded-full ${c.bar}`} style={{ width: `${confidencePct}%` }} />
                    </div>
                  </div>
                </div>
                ...
                {(persona.goals ?? []).length > 0 && (
                  <div className="mb-2.5">
                    ...
                    {(persona.goals ?? []).map((g) => (
                      <p key={g.text} className="text-xs text-secondary-foreground flex items-start gap-1.5 mb-0.5">
                        <TrendingUp className={`h-3 w-3 ${minimal ? "text-muted-foreground" : colorDark.text} mt-0.5 shrink-0`} />
                        {g.text}
                      </p>
                    ))}
                  </div>
                ...
                {(persona.pains ?? []).length > 0 && (
                  <div className="mb-3">
                    ...
                    {(persona.pains ?? []).map((p) => (
                      <p key={p.text} className="text-xs text-secondary-foreground flex items-start gap-1.5 mb-0.5">
                ...
                <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                  ...
                  {(persona.evidence ?? []).length} evidence source{(persona.evidence ?? []).length !== 1 ? "s" : ""}
                </div>
              </motion.div>
            );
          })}
      ...
      {icps.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          {icps.map((icp, i) => {
            const colorKey = ICP_COLORS[i % ICP_COLORS.length];
            const c = cMap[colorKey];
            const colorDark = colorMap[colorKey];
            const confidencePct = Math.round((icp.confidence ?? 0) * 100);
            const isInferred = icp.lifecycle_status === "inferred";
            const companySizes = icp.firmographics?.company_size?.join(", ") ?? "—";
            const industries = icp.firmographics?.industries?.join(", ") ?? "—";
            return (
              <motion.div
                key={icp.icp_id}
                initial={{ opacity: 0, y: 12 }}
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-2.5">
                    <div>
                      <h4 className="text-sm font-extrabold text-foreground capitalize">{icp.segment_type}</h4>
                      <div className="flex items-center gap-1.5 mt-1">
                        {isInferred && (
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${c.bg} ${c.text} ring-1 ${c.ring}`}>
                            Inferred
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl bg-card/50 ring-1 ring-border/30 p-3">
                    <div className="flex items-center gap-1.5 mb-1">
                      <Building2 className={`h-3 w-3 ${minimal ? "text-muted-foreground" : colorDark.text}`} />
                      <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Company Size</p>
                    </div>
                    <p className="text-xs font-semibold text-foreground">{companySizes}</p>
                  </div>
                  <div className="rounded-xl bg-card/50 ring-1 ring-border/30 p-3">
                    <div className="flex items-center gap-1.5 mb-1">
                      <Sparkles className={`h-3 w-3 ${minimal ? "text-muted-foreground" : colorDark.text}`} />
                      <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Industry</p>
                    </div>
                    <p className="text-xs font-semibold text-foreground">{industries}</p>
                  </div>
                </div>
              </motion.div>
            );
